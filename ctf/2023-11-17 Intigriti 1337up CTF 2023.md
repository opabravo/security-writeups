...
Tried to tamper X,Y,Z value via **Cheat Engine**, but it seems to have some type of protection
#### Tamper Assembly-CSharp.dll to jump higher

Fortunately this game was made by **Unity**, which can be easily found out by `Assembly-CSharp.dll`
...
```bash
ffuf -c -mc all -request order_fuzz.req -w /usr/share/seclists/Fuzzing/special-chars.txt -mr "Invalid characters detected"
```

![](attachment/7dac0c637b61266049a9f023cbba9518.png)

`&` and `+` were the only ones that weren't filtered

```bash
ffuf -c -mc all -request order_fuzz.req -w /usr/share/seclists/Fuzzing/special-chars.txt -fr "Invalid characters detected"
```

![](attachment/cfd155084df5faf5361c68f2a6e360a9.png)

I did not find any payload in wordlists that doesn't contain the banned characters

```bash
ffuf -c -request order_fuzz.req -w /usr/share/seclists/Fuzzing/special-chars.txt -mc all -mr "Invalid characters detected" -o banned_chars
```
...
bravosec@fsociety /media/sf_Kali/ctf/1337up @ wordlist = [l for l in $(cat /opt/wordlists/Auto_Wordlists/wordlists/ssti.txt /usr/share/seclists/Fuzzing/SSI-Injection-Jhaddix.txt /opt/wordlists/Auto_Wordlists/w
ordlists/ssti.txt ).splitlines() if not any(b for b in banned if b in l)]
...
> [!info]
> From red team perspective, don't put payloads in headers such as `User-Agent` since web logs will collet it as default
...
> [!NOTE]
> Run local tools like `sherlock` and `maigret` for further username enumeration (Github gist, various sources...etc)
...
Unfortunately `Profile ID : 29f83ddeaff255ae7842fae4ca83390d` wasn't a unique ID, 
...
In order to meet the condition, password must be : 

a base64 string from anonymous banner, and the length is between `21~30`
